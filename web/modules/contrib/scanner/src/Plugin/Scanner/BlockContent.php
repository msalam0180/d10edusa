...
use Drupal\block_content\Entity\BlockContent as DrupalBlockContent;

/**
 * Class BlockContent.
 *
 * @Scanner(
 *   id = "scanner_block_content",
 *   type = "block_content",
 * )
 */
class BlockContent extends Entity {
  ...
  public function search($field, $values) {
    ...
    foreach ($entities as $key => $id) {
      $entityAndId = 'block_content:' . $id;
      $block = DrupalBlockContent::load($id);
      $type = $block->bundle();
      $blockField = $block->get($fieldname);
      $fieldType = $blockField->getFieldDefinition()->getType();
      if (in_array($fieldType, ['text_with_summary', 'text', 'text_long'])) {
        $fieldValue = $blockField->getValue()[0];
        $title_collect[$entityAndId]['title'] = $block->label();
        ...
        $title_collect[$entityAndId]['field'] = $newValues;
      }
      elseif ($fieldType == 'string') {
        $title_collect[$entityAndId]['title'] = $block->label();
        preg_match($conditionVals['phpRegex'], $blockField->getString(), $matches, PREG_OFFSET_CAPTURE);
        $match = $matches[0][0];
        $replaced = preg_replace($conditionVals['phpRegex'], "<strong>$match</strong>", $blockField->getString());
        $title_collect[$entityAndId]['field'] = [$replaced];
      }
    }
    ...
    foreach ($entities as $key => $id) {
      ...
      if (in_array($fieldType, ['text_with_summary', 'text', 'text_long'])) {
        if ($values['language'] === 'all') {
          ...
          if (!isset($data[$entityAndId]['new_vid'])) {
            $data[$entityAndId]['old_vid'] = $block->getRevisionId();
            // Crete a new revision so that we can have the option of undoing it
            // later on.
            $block->setNewRevision(TRUE);
            $block->revision_log = t('Replaced %search with %replace via Scanner Search and Replace module.', ['%search' => $values['search'], '%replace' => $values['replace']]);
          }
        }
        else {
          $requested_lang = $values['language'];
          if ($block->hasTranslation($requested_lang)) {
            $block = $block->getTranslation($requested_lang);
            $blockField = $block->get($fieldname);
          }
          $fieldValue = $blockField->getValue()[0];
          // Replace the search term with the replace term.
        ...
        $data[$entityAndId]['new_vid'] = $block->getRevisionId();
      }
      elseif ($fieldType == 'string') {
        if (!isset($data[$entityAndId]['new_vid'])) {
          if ($values['language'] === 'all') {
            ...
            foreach ($all_languages as $langcode => $languageName) {
              ...
              $fieldValue = preg_replace($conditionVals['phpRegex'], $values['replace'], $blockField->getString());
              $block->$fieldname = $fieldValue;
            }
            $data[$entityAndId]['old_vid'] = $block->getRevisionId();
            $block->setNewRevision(TRUE);
            ...
            $requested_lang = $values['language'];
            if ($block->hasTranslation($requested_lang)) {
              // $blockField = $blockField->getTranslation($requested_lang);
              $block = $block->getTranslation($requested_lang);
              $blockField = $block->get($fieldname);
            }
            $fieldValue = preg_replace($conditionVals['phpRegex'], $values['replace'], $blockField->getString());
  ...
  public function undo($data) {
    $revision = \Drupal::entityTypeManager()->getStorage('block_content')->loadRevision($data['old_vid']);
