...
namespace Drupal\Tests\ebt_accordion\Functional;
...
class InstallTest extends BrowserTestBase {
  ...
  public function testInstallation() {
    $account = $this->drupalCreateUser(['access content', 'administer blocks']);
    $this->drupalLogin($account);

    $this->assertFalse($this->moduleHandler->moduleExists('ebt_accordion'));
    ...
    $this->assertTrue($this->moduleInstaller->install(['ebt_accordion']));
    \Drupal::service('config.installer')->installDefaultConfig('module', 'ebt_accordion');
    $this->reloadServices();
    $this->assertTrue($this->moduleHandler->moduleExists('ebt_accordion'));
    ...
    // Load the front page.
    $this->drupalGet('block/add/ebt_accordion');
